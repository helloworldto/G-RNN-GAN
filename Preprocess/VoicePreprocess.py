...
def VoicePreprocessAll(Data_Dir='data/MIR1K',mir1k_sr=16000,n_fft = 1024):
...
# Turn waves to spectrums
	stfts_mono_train, stfts_src1_train, stfts_src2_train = wavs_to_specs(wavs_mono = wavs_mono_train, wavs_src1 = wavs_src1_train, wavs_src2 = wavs_src2_train, n_fft = n_fft, hop_length = hop_length)
	...
	return stfts_mono_train, stfts_src1_train, stfts_src2_train,stfts_mono_valid, stfts_src1_valid, stfts_src2_valid

def VoiceProprocessFrame(stfts_mono_train, stfts_src1_train, stfts_src2_train,sample_frames = 10,batch_size = 64):
	data_mono_batch, data_src1_batch, data_src2_batch = sample_data_batch(stfts_mono=stfts_mono_train, stfts_src1=stfts_src1_train, stfts_src2=stfts_src2_train, batch_size=batch_size,sample_frames=sample_frames)
	...
	return x_mixed,y1,y2
 #change

def VoiceFinalFrameIn(mix,frequency,batch_size=64):
	tran=mix[:,batch_size*10*frequency:batch_size*10*(frequency+1),:]
	tran=np.reshape(tran,(batch_size,10,513),order='C')
	return tran
